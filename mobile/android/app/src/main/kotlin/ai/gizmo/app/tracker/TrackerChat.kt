...
@Composable
fun TrackerChat(serverUrl: String, onRefresh: () -> Unit, onDismiss: () -> Unit) {
    ...
    val ws = remember {
        GizmoWebSocket(serverUrl = serverUrl, wsPath = "/ws/tracker",
            onEvent = { event ->
                when (event) {
        ...
        Column(modifier = Modifier.fillMaxSize().padding(padding)) {
            ...
            Row(modifier = Modifier.fillMaxWidth().padding(8.dp), verticalAlignment = Alignment.CenterVertically) {
                TextField(value = inputText, onValueChange = { inputText = it },
                    placeholder = { Text("Ask about tasks...", color = TextDim) },
                ...
                IconButton(onClick = {
                    if (inputText.isNotBlank()) {
                        ...
                        ws.send(message = inputText); inputText = ""; generating = true
