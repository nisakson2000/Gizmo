...
<div
	...
	role="button"
	tabindex="0"
	class="group flex items-start gap-3 px-4 py-3 cursor-pointer transition-all hover:bg-bg-hover/50
		{isSelected ? 'bg-bg-hover/70 border-l-2 border-accent' : 'border-l-2 border-transparent'}"
>
	<!-- Pin indicator -->
	<div class="shrink-0 mt-0.5">
		{#if note.pinned}
			<svg class="w-3.5 h-3.5 text-accent" fill="currentColor" stroke="currentColor" viewBox="0 0 24 24">
	...
	<div class="flex-1 min-w-0">
		...
		{#if note.content}
			<p class="text-xs text-text-dim mt-0.5 truncate">{preview(note.content)}</p>
		{/if}
		{#if note.tags.length > 0}
			<div class="flex gap-1 mt-1.5">
				{#each note.tags.slice(0, 4) as tag}
					<span class="px-1.5 py-0.5 text-[10px] rounded-md bg-bg-tertiary/80 text-text-dim">{tag}</span>
				{/each}
			</div>
		{/if}
	</div>
