...
The basic unit is not a word but a **token** — roughly a word or word-piece. Common words like "the" are single tokens. Longer or rarer words get split: "unbelievable" might become "un" + "believ" + "able". Qwen3.5-9B has a vocabulary of about 150,000 tokens. Every input and output is converted to and from these tokens.
...
Qwen3.5-9B has **9 billion parameters**. Each parameter is a number (a floating-point value) stored in your GPU's memory. This is why VRAM is the primary hardware constraint — the weights must fit in memory for the model to run.

At full precision (16-bit floating point), 9 billion parameters would require about 18GB of memory. That fits on high-end consumer GPUs, but quantization still helps by freeing VRAM for other uses like the KV cache and TTS model.
...
The result: the 9B model fits in **~9.5GB** instead of 18GB. Quality loss is essentially zero — Q8_0 is considered lossless for all practical purposes.

| Quant | Size | VRAM Needed | Quality |
|-------|------|-------------|---------|
| Q4_K_M | ~5.5GB | ~7GB | Good |
| Q5_K_M | ~6.5GB | ~8GB | Very good |
| Q6_K | ~7.5GB | ~9GB | Near-lossless |
| **Q8_0** | **~9.5GB** | **~12GB** | **Lossless (used by Gizmo)** |
...
Qwen3.5-9B is a **thinking model** — it was trained to reason step-by-step inside `<think>...</think>` tags before producing its final response. This is similar to OpenAI's o1/o3 models or Anthropic's extended thinking.
